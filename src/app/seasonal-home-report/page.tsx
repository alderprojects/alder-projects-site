...
export function PropertyReportInner({ mode }: { mode: PropertyMode }) {
  ...
  if (gis?.geocoded) {
    ...
    if (!gis.inFloodZone) steps.push({ title: mode === 'buyer' ? 'Walk it with the inspector' : 'First, find the basics', why: mode === 'buyer' ? "You're hiring an inspector anyway. Tell them: where's the main water shutoff, age of the heat source, last septic pump-out, electrical panel age. The seller's disclosure won't tell you. The inspector will." : "Where's the water shutoff? Heat source? Septic cover? Most owners can't answer this on day one. Half an hour with a notebook and you'll know more about your house than 90% of buyers did at closing.", cost: 'Free', tag: 'good news', tagType: 'good', cta: 'Post your project', ctaUrl: qparams({ town: r?.snapshot?.town || '' }), guideLabel: 'Read: winterizing checklist', guideUrl: '/guides/winterizing-vermont-seasonal-home' })
  ...
  if (gis?.geocoded) {
    ...
    if (flags.length > 0) sum = (mode === 'buyer' ? "Here's what comes with this property. Heads up: " : "Yeah, we checked your address. Heads up: ") + flags.join(', and ') + (mode === 'buyer' ? '. Worth knowing before you make an offer.' : ". Here's what that actually means for you.")
    else sum = mode === 'buyer' ? "We checked the address. Nothing unusual \u2014 no flood zone, no shoreland, no wetlands. The property's got real flexibility, which means more options if you buy." : "We checked your address. Nothing unusual \u2014 no flood zone, no shoreland rules, no wetlands. You've got real flexibility here."
  }
    ...
    <div className="min-h-screen" style={{ fontFamily: F, background: '#fafaf8' }}>
      ...
      <main style={{ maxWidth: 480, margin: '0 auto', padding: '88px 24px 100px' }}>
        <div style={{ paddingTop: 60, paddingBottom: 32 }}>
          <p style={{ fontSize: 28, fontWeight: 300, color: '#1c1917', lineHeight: 1.3, margin: 0 }}>{mode === 'buyer' ? <>Buying this place?<br />Know what you&apos;re walking into.</> : <>What&apos;s this place<br />going to cost you?</>}</p>
          <p style={{ fontSize: 13, color: '#a8a29e', marginTop: 8 }}>{mode === 'buyer' ? <>Type the Vermont address. We&apos;ll check the maps and tell you what to factor into your offer.</> : <>Type any Vermont address. We&apos;ll check the maps and tell you what&apos;s going to be a project &mdash; and what isn&apos;t.</>}</p>
        </div>
        ...
        {r && (
          <div style={{ border: '1px solid #eeedeb', borderRadius: 12, background: 'white', overflow: 'hidden' }}>
            ...
            {mode !== 'buyer' && (<div style={{ background: '#fafaf8', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
            ...
            {mode === 'buyer' && (<div style={{ background: '#fafaf8', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
              <div>
                <p style={{ fontSize: 13, fontWeight: 500, color: '#1c1917', margin: 0 }}>Want the full buyer report?</p>
                <p style={{ fontSize: 11, color: '#78716c', margin: '2px 0 0' }}>Written brief covering everything above plus inspection talking points and offer adjustments. $79. Currently a waitlist.</p>
              </div>
              <a href="/buyers#waitlist" style={{ fontSize: 11, fontWeight: 600, color: 'white', background: '#1c1917', textDecoration: 'none', padding: '8px 16px', borderRadius: 999 }}>Get on waitlist →</a>
            </div>)}
          </div>
        )}
        {r && mode !== 'buyer' && (
