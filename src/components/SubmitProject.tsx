...
function SubmitProjectInner() {
  ...
  useEffect(()=>{
    ...
    if(budget && BUDGET_BANDS.includes(budget)) updates.budgetBand=budget
    if(timeline && TIMELINES.includes(timeline)) updates.timeline=timeline
    if(Object.keys(updates).length>0){
      setForm(p=>({...p,...updates}))
      if(source==='seasonal-report') setPrefillNotice('We pre-filled this from your property scan. Edit anything before submitting.')
      else if(source==='calculator') setPrefillNotice('We pre-filled this from your cost estimate. Edit anything before submitting.')
      else setPrefillNotice('Pre-filled. Edit anything before submitting.')
    }
  },[searchParams])
  ...
  return (
    <section id="submit-project" style={{backgroundColor:'#1C2B1A',padding:'clamp(56px,8vw,96px) 24px'}}>
      <div style={{maxWidth:'680px',margin:'0 auto'}}>
        ...
        {prefillNotice&&(<div style={{backgroundColor:'rgba(200,115,42,0.12)',border:'1px solid rgba(200,115,42,0.35)',borderRadius:'3px',padding:'12px 16px',marginBottom:'18px',fontSize:'13px',color:'#F5EFE0',lineHeight:1.5}}>{prefillNotice}</div>)}
...
export default function SubmitProject() {
  return (<Suspense fallback={null}><SubmitProjectInner/></Suspense>)
}
